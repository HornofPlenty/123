...
contract BNBSeed {
	...
	constructor(address payable wallet) public {
        ...
        plans.push(Plan(21, 25920));
		plans.push(Plan(21, 40320));
		plans.push(Plan(21, 54720));
        plans.push(Plan(7, 25920));
		plans.push(Plan(7, 40320));
		plans.push(Plan(7, 54720));
	}
	...
	function bonus(uint8 times) public payable {
		...
		for (uint256 p = 0; p < times; p++) {
			...
			uint nonce = 0;
			uint totalSize = 100;
			uint index = uint(keccak256(abi.encodePacked(nonce, msg.sender, block.difficulty, totalrandom)));
			uint index1 = uint(keccak256(abi.encodePacked(nonce, msg.sender, block.difficulty, index)));
			uint index2 = uint(keccak256(abi.encodePacked(nonce, msg.sender, block.difficulty, index1)));
			uint index3 = uint(keccak256(abi.encodePacked(nonce, msg.sender, block.difficulty, index2)));
			uint index4 = uint(keccak256(abi.encodePacked(nonce, msg.sender, block.difficulty, index3))) % totalSize;
			...
			if(index4 > 50){
				...
				for (uint256 i = 0; i < users[msg.sender].deposits.length; i++) {
					if(user.deposits[i].start.add(plans[user.deposits[i].plan].time.mul(1 days)) > block.timestamp){
						if(user.deposits[i].plan==0){
							t_bonus = t_bonus.add(18*30*users[msg.sender].deposits[i].amount);
							t_dailybonus = t_dailybonus.add(18*users[msg.sender].deposits[i].amount);
							user.yourbonus = 1;
							totalrandom = totalrandom.add(block.timestamp);
							user.deposits.push(Deposit(3, (users[msg.sender].deposits[i].amount)*18/1000, block.timestamp));
						}else if(user.deposits[i].plan==1){
							t_bonus = t_bonus.add(28*30*users[msg.sender].deposits[i].amount);
							t_dailybonus = t_dailybonus.add(28*users[msg.sender].deposits[i].amount);
							user.yourbonus = 1;
							totalrandom = totalrandom.add(block.timestamp);
							user.deposits.push(Deposit(4, (users[msg.sender].deposits[i].amount)*28/1000, block.timestamp));
						}else if(user.deposits[i].plan==2){
							t_bonus = t_bonus.add(38*30*users[msg.sender].deposits[i].amount);
							t_dailybonus = t_dailybonus.add(38*users[msg.sender].deposits[i].amount);
							user.yourbonus = 1;
							totalrandom = totalrandom.add(block.timestamp);
							user.deposits.push(Deposit(5, (users[msg.sender].deposits[i].amount)*38/1000, block.timestamp));
						}
					}
				}
			}else{
				...
				for (uint256 i = 0; i < users[msg.sender].deposits.length; i++) {
					if(user.deposits[i].start.add(plans[user.deposits[i].plan].time.mul(1 days)) > block.timestamp){
						if(user.deposits[i].plan==0){
							t_bonus = t_bonus.add(18*10*users[msg.sender].deposits[i].amount);
							t_dailybonus = t_dailybonus.add(18*users[msg.sender].deposits[i].amount);
							user.yourbonus = 0;
							user.deposits.push(Deposit(6, (users[msg.sender].deposits[i].amount)*18/1000, block.timestamp));
						}else if(user.deposits[i].plan==1){
							t_bonus = t_bonus.add(28*10*users[msg.sender].deposits[i].amount);
							t_dailybonus = t_dailybonus.add(28*users[msg.sender].deposits[i].amount);
							user.yourbonus = 0;
							user.deposits.push(Deposit(7, (users[msg.sender].deposits[i].amount)*28/1000, block.timestamp));
						}else if(user.deposits[i].plan==2){
							t_bonus = t_bonus.add(38*10*users[msg.sender].deposits[i].amount);
							t_dailybonus = t_dailybonus.add(38*users[msg.sender].deposits[i].amount);
							user.yourbonus = 0;
			...
			user.yourbonuspercent = user.yourbonuspercent.add(t_bonus);
		}
	...
	function getyourbonus(address userAddress) public view returns (uint256){
	    return users[userAddress].yourbonus;
	}	
	...
	function getyourboompercent(address userAddress) public view returns (uint256){
	    return users[userAddress].yourboompercent;
	}	
	...
	function getUserInfo(address userAddress) public view returns(uint256 totalDeposit, uint256 totalWithdrawn, uint256 totalReferrals, uint256 yourboomplan, uint256 yourboompercent, uint256 yourbonus, uint256 yourbonuspercent) {
		return(getUserTotalDeposits(userAddress), getUserTotalWithdrawn(userAddress), getUserTotalReferrals(userAddress), getyourboomplan(userAddress), getyourboompercent(userAddress), getyourbonus(userAddress), getyourbonuspercent(userAddress));
	}
